...
xCard.cardEvaluator.registerCard("Dead At The Mist", function(request, state) {
  state.callMethod( "addGameMessage", request, request.requestingPlayer.name + " breathed in death at the mist!" );
...
xCard.cardEvaluator.registerCard("Belonging To Myself", function(request, state) {
  state.callMethod( "addGameMessage", request, request.requestingPlayer.name + " crashed down with some phat philosophical banter!" );
...
xCard.cardEvaluator.registerCard("Assassins Of History", function(request, state) {
  state.callMethod( "addGameMessage", request, request.requestingPlayer.name + " stabbed a random player!" );
...
xCard.cardEvaluator.registerCard("Death At The Void", function(request, state) {
  state.callMethod( "addGameMessage", request, request.requestingPlayer.name + " fell on his sword ... Ouch! D:" );
...
xCard.cardEvaluator.registerCard( "Enemies And Officers", function( request, state ) {
  if( request.data.targets ) {
    ...
    state.callMethod( "addGameMessage", request, request.requestingPlayer.name + " shocked " + player.name );
